...
with session as s:
    ...
    payload["javax.faces.ViewState"] = parser.select_one("#j_id1:javax.faces.ViewState:0")["value"]
    print("Payload: " + str(payload))
    r = s.post("https://aurion-lille.isen.fr/faces/MainMenuPage.xhtml", params = payload)

    ### By now, we should have the room page and the link for exporting in CSV
    parser = BeautifulSoup(r.text.encode(encoding='UTF-8',errors='strict'), 'html.parser')
    ...
    payload["javax.faces.ViewState"] = parser.select_one("#j_id1:javax.faces.ViewState:0")["value"]
    print("Payload: " + str(payload))
    r = s.post("https://aurion-lille.isen.fr/faces/ChoixDonnee.xhtml", params = payload)
